...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('apellido'), 'has-success': fields.apellido && fields.apellido.valid }">
    <label for="apellido" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.cliente.columns.apellido') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.apellido" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('apellido'), 'form-control-success': fields.apellido && fields.apellido.valid}" id="apellido" name="apellido" placeholder="{{ trans('admin.cliente.columns.apellido') }}">
